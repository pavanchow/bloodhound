...
- **Reversibility.** Checking `goto(N)` against `trace[N]` for every N proves the debugger can reconstruct any historical state exactly. Separately, from random starting steps, forward K then back K is asserted equal to the starting snapshot, which proves reverse stepping is a true inverse of forward stepping and not a lossy approximation. Random programs exercise arithmetic, locals, globals, memory, a branch, and nested calls to depth three, so the undo logic is tested on every kind of mutation including frame pushes and pops.
